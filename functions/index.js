...
const { dialogflow,
        ...
        Image,
        Suggestions,
        MediaObject } = require('actions-on-google');
...
app.intent('Monica', (conv, { Monica }) => {
    ...
    if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT')) {
        ...
        conv.ask(`<speak> Please select from the follwoing options</speak>`)
        conv.ask(new Suggestions(['Bang', 'Freak', 'Competitive', ' Drunk', ' Monlander', 'Foosballs']));
    }
    //For Speakers
    if (!conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT')) {
        conv.ask(`<speak>Please switch to screen display based device</speak>`);
    } 
})
...
app.intent('Drunk', (conv) => {
    if (!conv.surface.capabilities.has('actions.capability.AUDIO_OUTPUT')) {
        conv.ask(`<speak>Please switch to device which support Audio output</speak>`);
    }
...
app.intent('media status', (conv) => {
    ...
    if (mediaStatus && mediaStatus.status === 'FINISHED') {
        response = 'Hope you enjoyed talking with me'
    }
    conv.ask(respose);
    conv.ask(new Suggestions(['Joey', 'Chandler', 'Monica', 'Ross', 'Phoebe', 'Rachel']));
})
